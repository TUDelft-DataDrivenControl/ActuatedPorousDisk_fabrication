...
for name in strainCalDataAPD_pd.keys():
    m = str2num(name[-7:])
    strainCalData['APD'][m] = np.array(strainCalDataBT_pd[name]['UpstreamThrust'])
...
ax.set(title="Strain Guage data for calibration weights", xlabel = "Weight / N", ylabel = "Strain measurement / mV")
...
apdplot = ax.errorbar(APD_data[0][APDsort], APD_data[1][APDsort] 
                    #   - APD_data[1][APDsort[-1]]
                      , APD_data[2][APDsort]
                      , fmt='x', elinewidth=1, markersize=5, c='tab:orange', capsize=4, label='APD - data points')
spdplot = ax.errorbar(SPD_data[0][SPDsort], SPD_data[1][SPDsort] 
                    #   - SPD_data[1][SPDsort[-1]]
                      , SPD_data[2][SPDsort]
                      , fmt='x', elinewidth=1, markersize=5, c='tab:olive', capsize=4, label='SPD - data points')
btfit, = ax.plot(BT_data[0][BTsort], bt0 + btJ * BT_data[0][BTsort], c='tab:blue', ls='--', label='BT - fit')
apdfit, = ax.plot(APD_data[0][APDsort], apd0 + apdJ * APD_data[0][APDsort], c='tab:orange', ls='--', label='APD - fit')
spdfit, = ax.plot(SPD_data[0][SPDsort], spd0 + spdJ * SPD_data[0][SPDsort], c='tab:olive', ls='--', label='SPD - fit')
ax.legend(handles=[btplot, btfit, apdplot, apdfit, spdplot, spdfit])
plt.show()