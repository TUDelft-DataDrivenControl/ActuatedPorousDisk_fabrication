# Mount the APD horizontally, keep the calibration weights at hand and run this script. 
...
for mass in cal_weights[:]:
    ...
    while ans != "y":
        ans = input(f"Put {mass} grams on the APD now. Did you do it? [y / N]")

    send_measurement_info(MCU_input)
    T, SG1, SG2 = receive_timeseries()

    fn = f"results\\01_APD_SG_force_cal\\{mass}g.npz" # Relative path and filename
